...
function connectToES() {
    ...
    var jqxhr = $.getJSON(loc + "/_cluster/state", function () {
        ...
        .fail(function () {
            $('#cluster').html('<strong>Error connecting to: ' + loc + '</strong>');
        });
}
...
function searchIndex() {
    ...
    $.ajax({
        ...
        data: $('#useLucene').is(':checked') ?
        {
            ...
            "from": $('#from').val(),
            "size": $('#size').val(),
            "lowercase_expanded_terms": $('#lowerCaseExpandedTerms').is(':checked'),
            "analyze_wildcard": $('#analyzeWildcard').is(':checked'),
            "search_type": $('#searchType').val()[0],
            "default_operator": $('#defOperator').val()[0]
        }
            : {
            "source": queryText,
            "sort": sortField,
